...
public class PerfilActivity extends AppCompatActivity {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    if(snapshot.hasChild("imagen")){
                        ...
                        Toast.makeText(PerfilActivity.this, "Selecciona una imagen de perfil", Toast.LENGTH_SHORT).show();
                    }
                }
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == CropImage.CROP_IMAGE_ACTIVITY_REQUEST_CODE){//ajustar imagen para perfil "recortar"
        ...
        if(TextUtils.isEmpty(nombres)){
            ...
            UserRef.child(CurrentUserId).updateChildren(map).addOnCompleteListener(new OnCompleteListener() {
                ...
                public void onComplete(@NonNull Task task) {
                    if(task.isSuccessful()){
                        Toast.makeText(PerfilActivity.this, "Guardadooo", Toast.LENGTH_SHORT).show();
                        EnviaralInicio();
                        dialog.dismiss();
                    }else{
